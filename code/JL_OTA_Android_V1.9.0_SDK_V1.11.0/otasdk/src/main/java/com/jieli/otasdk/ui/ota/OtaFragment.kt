...
import com.jieli.otasdk.ui.qr_code.QRCodeScanActivity
...
class OtaFragment : BaseFileFragment() {
    ...
    private val qrCodeLauncher = registerForActivityResult(StartActivityForResult()) { result ->
        ...
        if (result.resultCode == QRCodeScanActivity.QRCODE_HTTP) {
            val httpUrl = result.data?.getStringExtra(QRCodeScanActivity.QRCODE_HTTP_URL)
    ...
    private val connectionObserver = Observer<DeviceConnection> { otaConnection ->
    ...
    override fun onDestroy() {
        super.onDestroy()
        otaViewModel.otaConnectionMLD.removeObserver(connectionObserver)
    ...
    fun toQrScanFragment() {
        ...
        MainApplication.instance.initOpenCV()
        qrCodeLauncher.launch(Intent(requireContext(), QRCodeScanActivity::class.java))
    }
                    ...
                    AddFileWayWindow(requireContext()) { op ->
                        when (op) {
                            AddFileWayWindow.OP_SELECT_LOCAL_FILE -> {
                                //  跳到本地文件浏览
                                try {
                                    // "application/octet-stream" 在小米手机有兼容性问题，需要用 "application/*" 或者 ”*/*“
                                    filePickerLauncher.launch("application/*")
                                } catch (e: ActivityNotFoundException) {
                                    showTips(getString(R.string.read_file_failed))
                                }
                            }
                            ...
                            AddFileWayWindow.OP_SCAN_QR_CODE -> {
                                if(!PermissionUtils.hasSelfPermissions(requireContext(), Manifest.permission.CAMERA)){
                                    showPermissionTipsDialog(getString(R.string.system_set_camera))
                                }
    ...
    private fun observeCallback() {
        ...
        otaViewModel.otaConnectionMLD.observeForever(connectionObserver)
