...
class SettingsFragment : BaseFragment() {
    ...
    private fun initUI() {
        ...
        UIHelper.updateSettingsTextUI(
            ...
            if (viewModel.isAutoTest() || binding.switchAutoTest.swBtn.isChecked) return@updateSettingsTextUI
    ...
    private fun refreshAutoTestUI(cfg: OtaConfiguration) {
        ...
        JL_Log.d(tag, "refreshAutoTestUI", "configuration : $isAutoTest")
    ...
    //更新当前通讯方式UI
    private fun updateCommunicationChannelUI(connectWay: Int, mtu: Int = -1) {
        UIHelper.updateSettingsCheckUI(
            ...
            updateCommunicationChannelUI(BluetoothConstant.PROTOCOL_TYPE_BLE)
            compareOtaConfiguration()
        }
        UIHelper.updateSettingsCheckUI(
            binding.viewSppWay,
            ...
            updateCommunicationChannelUI(BluetoothConstant.PROTOCOL_TYPE_SPP)
            compareOtaConfiguration()
        }
        UIHelper.updateSettingsCheckUI(
            binding.viewGattOverBrEdrWay,
            getString(R.string.communication_way_gatt_over_br_edr),
            connectWay == BluetoothConstant.PROTOCOL_TYPE_GATT_OVER_BR_EDR
        ) {
            updateCommunicationChannelUI(BluetoothConstant.PROTOCOL_TYPE_GATT_OVER_BR_EDR)
            compareOtaConfiguration()
        }
        val isAllowAdjustMtu = connectWay == BluetoothConstant.PROTOCOL_TYPE_BLE
        binding.viewAdjustMtu.root.apply {
            isEnabled = isAllowAdjustMtu
            isClickable = isAllowAdjustMtu
        }
        binding.viewAdjustMtu.tvTitle.setTextColor(
            ContextCompat.getColor(
                requireContext(), if (isAllowAdjustMtu) R.color.text_color
                else R.color.black_66242424
            )
        )
        binding.viewAdjustMtu.tvValue.setTextColor(
            ContextCompat.getColor(
                requireContext(),
                if (isAllowAdjustMtu) R.color.gray_838383 else R.color.gray_66838383
            )
        )

        var requestMtu = mtu
        if (!isAllowAdjustMtu) {
            requestMtu = BluetoothConstant.BLE_MTU_MAX
        }
        if (requestMtu >= 0) {
            UIHelper.updateSettingsTextUI(
                binding.viewAdjustMtu,
                getString(R.string.adjust_ble_mtu),
                formatBleMtu(requestMtu)
            ) {
    ...
    private fun refreshUI(cfg: OtaConfiguration) {
        ...
        /*通讯方式*/
        updateCommunicationChannelUI(cfg.connectionWay, cfg.bleMtu)
    }
    ...
    private fun getCurrentOtaConfiguration(): OtaConfiguration {
        return OtaConfiguration().apply {
            ...
            } else if (UIHelper.getCheckValue(binding.viewGattOverBrEdrWay)) {
    ...
    private fun compareOtaConfiguration(cfg: OtaConfiguration = getCurrentOtaConfiguration()) {
        viewModel.updateSettingConfigure(cfg)
    }
